...
public class PassiveAbility_ShopPassiveLogic7 : PassiveAbilityBase
{
  ...
  {
    base.OnKill(target);
    if (this.owner.currentDiceAction == null || this.owner.currentDiceAction.card.GetSpec().Ranged != CardRange.Far || this.owner.currentDiceAction.cardBehaviorQueue.Count <= 0)
      return;
