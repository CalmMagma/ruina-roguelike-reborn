...
public class LogAssetBundleManager : Singleton<LogAssetBundleManager>
{
  ...
  {
    this.bundles.AddRange((IEnumerable<AssetBundle>) bundlelist);
  }
  ...
  {
    LogAssetBundleManager.GameObjectBundleCache objectBundleCache = this.GObjList.Find((Predicate<LogAssetBundleManager.GameObjectBundleCache>) (x =>
    {
      if (!(x.objname == name))
        return false;
